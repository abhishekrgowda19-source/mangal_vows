...
from models import User, Agent, Dispute
...
@agent_bp.route("/agent-raise-ticket", methods=["GET", "POST"])
def agent_raise_ticket():
    if session.get("role") != "agent":
        return redirect(url_for("agent_bp.agent_login"))
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        return redirect(url_for("agent_bp.agent_my_tickets"))

    return render_template("raise_ticket.html", agent=agent)


# ── Agent My Tickets ──────────────────────────────────

@agent_bp.route("/agent-my-tickets")
def agent_my_tickets():
    ...
    return render_template("my_tickets.html", tickets=tickets, agent=agent)
